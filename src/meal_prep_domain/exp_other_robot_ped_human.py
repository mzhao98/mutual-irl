...
from robot_5_pedbirl_pragplan import Robot as Pragmatic_Robot_Human
...
ACTION_TO_PRECONDITION = {CHOP_CHICKEN: [GET_CUTTING_BOARD], POUR_BROTH: [GET_BOWL], GET_BOWL: [GET_CUTTING_BOARD], GET_PLATE: [GET_CUTTING_BOARD],
...
class Meal_Prep():
    ...
    def step(self, joint_action):
        ...
        team_rew, robot_rew, human_rew = -20,0,0

        if robot_action is not None and robot_action not in self.state_actions_completed:
            preconditions_list = ACTION_TO_PRECONDITION[robot_action]
            if set(preconditions_list).issubset(self.state_actions_completed):
                robot_action_successful = True
                self.state_actions_completed.append(robot_action)
                robot_rew += self.robot.ind_rew[robot_action]
                # robot_rew += self.human.ind_rew[robot_action]


        if human_action is not None and human_action not in self.state_actions_completed:
    ...
    def state_to_tuple(self, state_actions_completed):
        return tuple(state_actions_completed)
    ...
    def enumerate_states(self):
        ...
        while stack:
            ...
            for idx, action in enumerate(actions):
                ...
                # print("STATE", state)
                # print("ACTION", action)
                # print("TEAM REWARD", team_rew)
                # print("ROBOT REWARD", robot_rew)
...
def run_k_rounds(exp_num, task_reward, seed, h_alpha, update_threshold, random_human, replan_online, use_exploration, task_type):
    ...
    permutes = [[-2,-2,-2,0,9,9,10], [9,9,10,0,-2,-2,-2]]
    ...
    env = Meal_Prep(robot, human, starting_objects)
    ...
    human = Pragmatic_Robot_Human(team_rew, robot_rew, human_rew, starting_objects, robot_knows_human_rew=False,
                                  permutes=permutes,
                                  vi_type='cvi', is_collaborative_human=True, update_threshold=update_threshold)
