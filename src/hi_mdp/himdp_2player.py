...
from true_human_agent import Human_Model
...
class HiMDP():
    ...
    def create_human_model(self):
        self.human_model = Human_Model(self.human_rew, self.human_depth)


    def reset(self):
        self.initialize_game()
        self.set_robot_rew((0.9, -0.9, 0.1, 0.3))
        self.joint_actions = self.get_all_joint_actions()
    ...
    def get_all_joint_actions(self):
        all_joint_actions = []
        for p1_color in COLOR_LIST:
            for p2_color in COLOR_LIST:
                all_joint_actions.append((p1_color, p2_color))
        return all_joint_actions

    def get_possible_joint_actions_from_state(self, state):
        num_remaining = 0
        for color in COLOR_LIST:
            num_remaining += (state[ENV_IDX][color])
    ...
    def check_is_valid_joint_action(self, current_state, joint_action):
        ...
        for player_idx in range(NUM_PLAYERS):
            ...
            num_remain = state[ENV_IDX][action_color]
            # No more colors remain, invalid action
            if num_remain <= 0:
                is_valid = False
            else:
                state[ENV_IDX][action_color] -= 1
        ...
        while stack:
            ...
            for idx, action in enumerate(available_robot_actions):
                ...
                for h_idx, h_action in enumerate(available_human_actions):
                    ...
                    G.add_edge(state_tup, new_state_tup, weight=current_human_rew, action=(action, h_action))
