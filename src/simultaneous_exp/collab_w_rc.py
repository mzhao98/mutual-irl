...
def run_k_rounds(exp_num, task_reward, h_alpha=0.0):
    ...
    # Test 2 greedy
    robot = Collaborative_Human(robot_rew, human_rew, starting_objects, 0)
    human = Collaborative_Human(human_rew, robot_rew, starting_objects, 0)
    env = Simultaneous_Cleanup(robot, human, starting_objects)
...
def run_experiment():
    ...
    # show the plot
    fig.tight_layout()
    plt.savefig("simultaneous_collab_w_rc_100_violin.png")
    plt.show()
    plt.close()
    ...
    fig.tight_layout()
    plt.savefig("simultaneous_collab_w_rc_100_bar.png")
    plt.show()
    plt.close()
...
def run_experiment_h_alpha():
    ...
    Y = np.array([h_alpha_to_avg_percent_cvi[d] for d in h_alpha_to_avg_percent_cvi])
    ...
    # fig, ax = plt.subplots(figsize=(5, 5))
    plt.title('Collaboration w/ \nIndividual Constraints', fontsize=16)
    plt.plot(X, Y, 'k-')
    plt.fill_between(X, Y - Ystd, Y + Ystd, alpha=0.5, edgecolor='#CC4F1B', facecolor='#FF9848')
    plt.ylabel("Percent of Optimal")
    plt.xlabel("Percent Suboptimal Human Behavior")

    plt.legend()
    plt.savefig("simultaneous_assistance_100_h_alph.png")
    plt.show()
