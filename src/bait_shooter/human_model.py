...
ACTION_LIST = [POSITION_TO_BAIT, ACT_AS_BAIT, POSITION_TO_SHOOT, SHOOT, WAIT]
...
class Suboptimal_Collaborative_Human:
    ...
    def act(self, state, round_no=0):
        ...
        for (candidate_r_act, candidate_h_act) in self.possible_actions:
            # print("state", state)
            # print("concat state", np.concatenate(state))
            # print("ACT_AS_BAIT in state", ACT_AS_BAIT in np.concatenate(state))
            # print("candidate_r_act", candidate_r_act==SHOOT)
            # print("candidate_h_act", candidate_h_act==SHOOT)
