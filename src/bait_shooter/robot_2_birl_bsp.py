...
class Robot:
    ...
    def update_based_on_h_action(self, current_state, robot_action, human_action):
        ...
        for idx in self.beliefs:
            ...
            for r_action in self.possible_actions:
                ...
                if r_action is not None and r_action not in np.concatenate(copy_current_state):
                    preconditions_list = ACTION_TO_PRECONDITION[r_action]
                    if set(preconditions_list).issubset(copy_current_state[0]):
                        if r_action == SHOOT:
                            if ACT_AS_BAIT in np.concatenate(copy_current_state):
                                copy_current_state[0].append(r_action)
                ...
                if human_action is not None and human_action not in np.concatenate(copy_current_state):
                    preconditions_list = ACTION_TO_PRECONDITION[human_action]
                    if set(preconditions_list).issubset(copy_current_state[1]):
                        if human_action == SHOOT:
                            if ACT_AS_BAIT in np.concatenate(copy_current_state):
                                copy_current_state[1].append(human_action)
            ...
            for possible_action in h_reward_hypothesis:
                if possible_action not in current_state:
                    ...
                    for r_action in self.possible_actions:
                        ...
                        if possible_action is not None and possible_action not in np.concatenate(copy_current_state):
                            preconditions_list = ACTION_TO_PRECONDITION[possible_action]
                            if set(preconditions_list).issubset(copy_current_state[1]):
                                if possible_action == SHOOT:
                                    if ACT_AS_BAIT in np.concatenate(copy_current_state):
                                        copy_current_state[1].append(possible_action)
    ...
    def get_info_gain(self, state, human_action_to_prob_for_state):
        ...
        for robot_action in self.possible_actions:
            if robot_action is not None and robot_action in np.concatenate(state):
                continue
            robot_action_to_info_gain[robot_action] = 0
            for human_action in self.possible_actions:
                if human_action is not None and human_action in np.concatenate(state):
                    prob_a_h_t = 0
                elif set(ACTION_TO_PRECONDITION[human_action]).issubset(state[1]) is False:
                    prob_a_h_t = 0
                else:
                ...
                for h_reward_idx in self.beliefs:
                    ...
                    for h_act in possible_human_action_to_prob:
                        ...
                        elif set(ACTION_TO_PRECONDITION[h_act]).issubset(next_state[1]) is False:
                            human_action_to_prob[h_act] = 0
                ...
                for human_action_next_state in self.possible_actions:
                    if human_action_next_state is not None and human_action_next_state not in np.concatenate(next_state):
                        ...
                        if set(ACTION_TO_PRECONDITION[human_action_next_state]).issubset(next_state[1]) is False:
                            info_gain = 0
                        else:
