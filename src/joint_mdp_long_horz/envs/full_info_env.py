...
class Joint_MDP:
    ...
    def step(self, joint_action):
        ...
        for player_idx in range(NUM_PLAYERS):
            ...
            if action_color == NO_MOVE:
                ...
                if num_remain > 0:
                    ...
                    self.state[PIDX_to_HIDX[player_idx]].append(action_color)
        ...
        if done:
            ...
            for p_idx in range(NUM_PLAYERS):
                player_rew = 0
                player_history = self.state[PIDX_to_HIDX[p_idx]]
                for action_color in player_history:
                    rew = self.player_to_reward_function[p_idx][action_color]
                    player_rew += rew + 1

                total_reward += player_rew
    ...
    def enumerate_states(self):
        ...
        while stack:
            ...
            #     pdb.set_trace()
            # if state_tup == (1, 1, 0, 0, 1, 2, 0, 0, 1, 1, 0, 0, 1, 2, 0, 0, 2, 3, 0, 1):
            #     pdb.set_trace()
            ...
            for idx, action in enumerate(available_actions):
                ...
                # if rew < 0:
                #     rew = rew * 10
                # if state == [[1, 1, 0, 0], [1, 2, 0, 0], [1, 1, 0, 0], [1, 2, 0, 0], [2, 3], [0, 1]]:
                #     pdb.set_trace()
                G.add_edge(state_tup, new_state_tup, weight=rew, action=action)
    ...
    def rollout_full_game_vi_policy(self, policy, savename='example_rollout.png'):
        ...
        while not done:
            ...
            if is_valid is False:
                print(f"joint_action_selected = {joint_action_selected}, invalid")
                pdb.set_trace()
