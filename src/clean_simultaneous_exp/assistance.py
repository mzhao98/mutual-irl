...
def run_k_rounds(exp_num, task_reward, h_alpha=0.0):
    ...
    robot_rew = {keyname:0 for keyname in human_rew}
    ...
    permutes = list(itertools.permutations(human_rew.values()))
...
def run_experiment():
    ...
    ax.set_title('Assistance', fontsize=16)
    ...
    # show the plot
    fig.tight_layout()
    plt.savefig("assistance_100_violin.png")
    plt.show()
    plt.close()
    ...
    fig.tight_layout()
    plt.savefig("assistance_100_bar.png")
    plt.show()
    plt.close()
...
def run_experiment_h_alpha():
    ...
    h_alpha_to_std_percent_cvi = {}
    for h_alpha in np.arange(0.0, 1.1, step=0.1):
    # for h_alpha in [0.9]:
        cvi_percents = []
        stdvi_percents = []
        ...
        h_alpha_to_std_percent_cvi[h_alpha] = np.round(np.std(cvi_percents), 2)

        print(f"h_alpha = {h_alpha}: cvi_percents = ", cvi_percents)
    X = [d for d in h_alpha_to_avg_percent_cvi]
    Y = [h_alpha_to_avg_percent_cvi[d] for d in h_alpha_to_avg_percent_cvi]
    Ystd = np.array([h_alpha_to_std_percent_cvi[d] for d in h_alpha_to_std_percent_cvi])
    print("h_alpha_to_avg_percent_cvi", h_alpha_to_avg_percent_cvi)
    ...
    plt.xlabel("Percent Suboptimal Human Behavior")

    plt.legend()
    plt.savefig("assistance_100_h_alph.png")
    plt.show()
