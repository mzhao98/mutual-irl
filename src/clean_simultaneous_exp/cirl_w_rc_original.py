...
class Simultaneous_Cleanup():
    ...
    def rollout_multiround_game_two_agents(self, num_rounds):
        ...
        # print("multiround_belief_history", multiround_belief_history)
        # pdb.set_trace()
...
def run_k_rounds(exp_num, task_reward, h_alpha=0.0):
    ...
    reward_for_all_rounds = env.rollout_multiround_game_two_agents(num_rounds=4)
    ...
    return cvi_percent_of_opt_team, stdvi_percent_of_opt_team, cvi_percent_of_opt_human, stdvi_percent_of_opt_human, \
           cvi_percent_of_opt_robot, stdvi_percent_of_opt_robot, altruism_case, percent_opt_each_round, multiround_belief_history, robot_rew, human_rew
...
def run_experiment():
    ...
    multiround_belief_history = None
    with Pool(processes=100) as pool:
        ...
        for result in k_round_results:
            ...
            cvi_percent_of_opt_robot, stdvi_percent_of_opt_robot, altruism_case, percent_opt_each_round, multiround_belief_history, robot_rew, human_rew = result
    ...
    robot_belief_of_human_over_time = []
    true_rew_idx = None
    for round_no in multiround_belief_history:
        robot_history_of_human_beliefs = multiround_belief_history[round_no]
        for i in range(len(robot_history_of_human_beliefs)):
            beliefs_i = robot_history_of_human_beliefs[i]

            object_keys = list(robot_rew.keys())
            true_human_rew_dict = {object_keys[j]: list(human_rew)[j] for j in range(len(object_keys))}
            if true_rew_idx is None:
                for idx in beliefs_i:
                    if true_human_rew_dict == beliefs_i[idx]['reward_dict']:
                        true_rew_idx = idx
                        break

            true_prob = robot_history_of_human_beliefs[true_rew_idx][i]['prob']
            robot_belief_of_human_over_time.append(true_prob)

    plt.plot(range(len(robot_belief_of_human_over_time)), robot_belief_of_human_over_time)
    plt.xlabel("Timestep")
    plt.ylabel("Belief of true human rew")
