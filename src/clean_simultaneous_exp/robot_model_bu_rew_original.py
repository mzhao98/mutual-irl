...
class Robot:
        ...
        for idx in self.beliefs:
            ...
            for possible_action in h_reward_hypothesis:
                if current_state[possible_action] > 0:
                    sum_Z += h_reward_hypothesis[possible_action]
                    all_possible_rews.append(h_reward_hypothesis[possible_action])


            # print("all_possible_rews = ", all_possible_rews)
            # prob_obs_given_theta_normalized = 0.1
            # if human_only_rew_for_action == max(all_possible_rews):
            #     prob_obs_given_theta_normalized = 0.9

            prob_theta = self.beliefs[idx]['prob']
            prob_obs_given_theta_normalized = human_only_rew_for_action/sum_Z
    ...
    def get_joint_action_under_greedy_hypothesis(self, current_state_remaining_objects, human_reward):
        best_joint_act = []
        max_reward = -100
        ...
        for joint_act in self.possible_joint_actions:
            ...
            _, (team_r, robot_r, human_r), _ = self.human_step_given_state(current_state_remaining_objects, joint_act,
                                                                           human_reward)
            # print("candidate_rew", candidate_rew)
            candidate_rew = team_r + human_r
            if candidate_h_act is not None:
                if candidate_rew == max_reward:
                    if (candidate_r_act, candidate_h_act) not in best_joint_act:
                        best_joint_act.append((candidate_r_act, candidate_h_act))

                elif candidate_rew > max_reward:
                    max_reward = candidate_rew
                    best_joint_act = [(candidate_r_act, candidate_h_act)]

        if len(best_joint_act) == 0:
            j_action = (None, None)
        else:
            # h_action = best_human_act[np.random.choice(range(len(best_human_act)))]
            j_action = best_joint_act[0]

        possible_j_action_to_prob = {}
        for joint_act in self.possible_joint_actions:
            candidate_r_act = joint_act['robot']
            candidate_h_act = joint_act['human']
            if (candidate_r_act, candidate_h_act) == j_action:
                possible_j_action_to_prob[(candidate_r_act, candidate_h_act)] = 1
            else:
                possible_j_action_to_prob[(candidate_r_act, candidate_h_act)] = 0

        return possible_j_action_to_prob

    def get_joint_action_under_collaborative_hypothesis(self, current_state_remaining_objects, human_reward):
        best_joint_act = []
        max_reward = -100
        # print("self.possible_joint_actions", self.possible_joint_actions)
        for joint_act in self.possible_joint_actions:
        ...
        possible_j_action_to_prob = {}
        for joint_act in self.possible_joint_actions:
            if joint_act == j_action:
                possible_j_action_to_prob[joint_act] = 1
            else:
                possible_j_action_to_prob[joint_act] = 0

        return possible_j_action_to_prob

    def get_joint_action_under_hypothesis(self, current_state_remaining_objects, human_reward):
        # print("human_reward", human_reward)
        if self.is_collaborative_human is True:
            possible_j_action_to_prob = self.get_joint_action_under_collaborative_hypothesis(current_state_remaining_objects, human_reward)
        else:
            possible_j_action_to_prob = self.get_joint_action_under_greedy_hypothesis(current_state_remaining_objects, human_reward)
        return possible_j_action_to_prob
            ...
            for s in range(n_states):
                ...
                if len(current_state_remaining_objects) == 0 or sum(current_state_remaining_objects.values()) == 0:
                    ...
                    for action_idx in self.idx_to_action:
                        ...
                        possible_h_action_to_prob = self.get_joint_action_under_hypothesis(current_state_remaining_objects, most_probable_h_reward)
                        h_prob = possible_h_action_to_prob[(r_act, h_act)]
        ...
        for s in range(n_states):
            ...
            if len(current_state_remaining_objects) == 0 or sum(current_state_remaining_objects.values()) == 0:
                ...
                for action_idx in self.idx_to_action:
                    ...
                    possible_h_action_to_prob = self.get_joint_action_under_hypothesis(current_state_remaining_objects,
