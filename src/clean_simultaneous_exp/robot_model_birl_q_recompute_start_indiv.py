...
class Robot:
    ...
    def hmr_reset(self, hmr_starting_objects):
        ...
        self.hmr_possible_joint_actions = self.hmr_get_all_possible_joint_actions()
    ...
    def update_based_on_h_action(self, current_state, robot_action, human_action):
        ...
        joint_action_idx = self.hmr_action_to_idx[human_action]
        ...
        for idx in self.beliefs:
            ...
            human_only_rew_for_action = q_values_table[state_idx, joint_action_idx]
            # human_only_rew_for_action = 0
            ...
            exp_q_value_for_obs = np.exp(self.beta * human_only_rew_for_action)
            # print("exp_q_value_for_obs", exp_q_value_for_obs)
            print(f"Q-value: idx = {idx}, human_only_rew_for_action = {human_only_rew_for_action}, exp_q_value_for_obs = {np.round(exp_q_value_for_obs, 2)}")
            # print("h_reward_hypothesis", h_reward_hypothesis)
            # exp_q_value_for_obs = q_value_for_obs
        ...
        for idx in self.beliefs:
            prob_theta = self.beliefs[idx]['prob']
            prob_obs_given_theta_normalized = dict_prob_obs_given_theta[idx]
            self.beliefs[idx]['prob'] = prob_theta * prob_obs_given_theta_normalized * (1/self.timestep_for_belief)
            # print(f"idx = {idx}, prob before normalize = {np.round(self.beliefs[idx]['prob'], 2)}")
            normalization_denominator += prob_theta * prob_obs_given_theta_normalized
    ...
    def hmr_get_all_possible_joint_actions(self):
        ...
        self.hmr_possible_joint_actions =  possible_joint_actions
    ...
    def hmr_enumerate_states(self, hmr_starting_objects):
        ...
        while stack:
            ...
            for idx, action in enumerate(actions):
                ...
                # add edge to graph from current state to new state with weight equal to reward
                if action['human'] is None:
                    G.add_edge(state_tup, new_state_tup, weight=team_rew, action='None')
                else:
                    G.add_edge(state_tup, new_state_tup, weight=team_rew, action=action['human'])
        ...
        # pdb.set_trace()
        action_to_idx = {action['human']: i for i, action in enumerate(actions)}
        idx_to_action = {i: action['human'] for i, action in enumerate(actions)}
    ...
    def get_robot_action_for_human_candidate(self, current_state_remaining_objects, human_reward):
        state_remaining_objects = copy.deepcopy(current_state_remaining_objects)
        best_human_act = []
        max_reward = -100
        r_action = None
        ...
        best_reward_for_robot_action = {}

        for joint_act in self.hmr_possible_joint_actions:
            candidate_r_act = joint_act['robot']
            candidate_h_act = joint_act['human']
            # joint_act = {'robot': candidate_r_act, 'human': candidate_h_act}
            # print("joint_act", joint_act)
            ...
            if candidate_r_act not in best_reward_for_robot_action:
                best_reward_for_robot_action[candidate_r_act] = candidate_rew
            else:
                if candidate_rew > best_reward_for_robot_action[candidate_r_act]:
                    best_reward_for_robot_action[candidate_r_act] = candidate_rew
        ...
        possible_r_action_to_prob = {}
        for candidate_r_act in self.hmr_possible_actions:
            if candidate_r_act == r_action:
                possible_r_action_to_prob[candidate_r_act] = 1
            # elif candidate_r_act is None:
            #     possible_r_action_to_prob[candidate_r_act] = 1
            # elif best_reward_for_robot_action[candidate_r_act] == max_reward:
            #     possible_r_action_to_prob[candidate_r_act] = 1
            # else:
            #     possible_r_action_to_prob[candidate_r_act] = 0

        # print("possible_r_action_to_prob", possible_r_action_to_prob)
        return possible_r_action_to_prob
        # return r_action
            ...
            for s in range(self.n_states):
                ...
                if len(current_state_remaining_objects) == 0 or sum(current_state_remaining_objects.values()) == 0:
                    ...
                    for action_idx in self.idx_to_action:
                        ...
                        for h_reward_idx in self.beliefs:
                            # print(f"h_reward_idx = {h_reward_idx}")
                            h_reward_hypothesis = self.beliefs[h_reward_idx]['reward_dict']
                            probability_of_hyp = self.beliefs[h_reward_idx]['prob']
    ...
    def human_candidate_value_iteration(self, human_rew_dict):
        ...
        for i in range(self.maxiter):
            ...
            for s in range(n_states):
                ...
                if len(current_state_remaining_objects) == 0 or sum(current_state_remaining_objects.values()) == 0:
                    ...
                    # compute new Q values
                    #
                    r_act_to_prob = self.get_robot_action_for_human_candidate(current_state_remaining_objects, human_rew_dict)

                    Q_value_summation = 0
                    for h_action_idx in self.hmr_idx_to_action:
                        # pdb.set_trace()
                        # check joint action
                        h_act = self.hmr_idx_to_action[h_action_idx]

                        for best_r_action in r_act_to_prob:
                            prob = r_act_to_prob[best_r_action]
                            # if best_r_action not in self.hmr_idx_to_action:
                            #     continue
                            #     # r_act = None
                            # else:f
                            # if best_r_action is None:
                            #     best_r_action = 'None'
                            #     print("self.hmr_idx_to_action", self.hmr_idx_to_action)
                            # r_act = self.hmr_idx_to_action[best_r_action]
                            r_act = best_r_action
                            joint_action = {'robot': r_act, 'human': h_act}
                            ...
                            Q[s, h_action_idx] += prob * (r_sa + (self.human_gamma * vf[s11]))

                        # Q_value_summation += (r_sa + (self.human_gamma * vf[s11])) * r_prob

                        # for r_action_idx in self.hmr_idx_to_action:
                        #     r_act = self.hmr_idx_to_action[r_action_idx]
                        #     if best_r_action != r_act:
                        #         continue
                        #     joint_action = {'robot': r_act, 'human': h_act}
                            ...
                            #         current_state_remaining_objects, joint_action, human_rew_dict)
                            #     r_sa = robot_rew + human_rew + team_rew
                            #     # r_sa = reward_values[r_action_idx]
                            #     # next_state = next_states[r_action_idx]
                            #
                            # else:
                            ...
                            #
                            # Q_value_summation += (r_sa + (self.human_gamma * vf[s11])) * r_prob
                                # r_prob = robot_action_to_prob[r_act]
                                # s11 = self.hmr_state_to_idx[self.state_to_tuple(next_state)]

                                # Q_value_summation += (r_sa + (self.human_gamma * vf[s11])) * r_prob
                        # action_idx = self.action_to_idx[(r_act, h_action)]
                        # Q[s, h_action_idx] = r_sa + (self.human_gamma * vf[s11])
        ...
        for s in range(n_states):
            ...
            if len(current_state_remaining_objects) == 0 or sum(current_state_remaining_objects.values()) == 0:
                ...
                # compute new Q values
                # best_r_action = self.get_robot_action_for_human_candidate(current_state_remaining_objects, human_rew_dict)

                r_act_to_prob = self.get_robot_action_for_human_candidate(current_state_remaining_objects,
                                                                          human_rew_dict)
                ...
                #         joint_action = {'robot': r_act, 'human': h_act}
                    ...
                    #     Q_value_summation += (r_sa + (self.human_gamma * vf[s11])) * r_prob
                    # # action_idx = self.action_to_idx[(r_act, h_action)]
                    # Q[s, h_action_idx] = Q_value_summation
