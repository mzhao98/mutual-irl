...
class Simultaneous_Cleanup():
    ...
    def rollout_multiround_game_two_agents(self, num_rounds):
        ...
        return total_reward, human_only_reward, robot_only_reward, multiround_belief_history
...
def run_k_rounds(exp_num, task_reward, h_alpha=0.0):
    ...
    robot = Robot(robot_rew, human_rew, starting_objects, permutes=None, vi_type='cvi', is_collaborative_human=False)
    ...
    robot = Robot(robot_rew, None, starting_objects, permutes=None, vi_type='cvi', is_collaborative_human=False)
    human = Greedy_Human(human_rew, robot_rew, starting_objects, h_alpha)
    robot.setup_value_iteration()
    env = Simultaneous_Cleanup(robot, human, starting_objects)
    cvi_rew, cvi_human_rew, cvi_robot_rew, multiround_belief_history = env.rollout_multiround_game_two_agents(num_rounds=6)
    print("CVI final_team_rew = ", cvi_rew)

    robot = Robot(robot_rew, human_rew, starting_objects, permutes=None, vi_type='stdvi', is_collaborative_human=False)
    human = Greedy_Human(human_rew, robot_rew, starting_objects, h_alpha)
    robot.setup_value_iteration()
    env = Simultaneous_Cleanup(robot, human, starting_objects)
...
def run_experiment():
    ...
    # show the plot
    fig.tight_layout()
    plt.savefig(f"cirl_w_h_rc_{num_exps}_violin.png")
    plt.show()
    plt.close()
    ...
    fig.tight_layout()
    plt.savefig(f"cirl_w_h_rc_{num_exps}_bar.png")
    plt.show()
    plt.close()
