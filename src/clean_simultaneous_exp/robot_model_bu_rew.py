        ...
        for idx in self.beliefs:
            ...
            for r_action in self.possible_actions:
                ...
                if current_state[r_action] > 0:
                    human_only_rew_for_action += (h_reward_hypothesis[human_action] + self.ind_rew[r_action])

            sum_Z = 0
            all_possible_rews = []
            for possible_action in h_reward_hypothesis:
                if current_state[possible_action] > 0:
                    # sum_Z += h_reward_hypothesis[possible_action] + self.ind_rew[robot_action]
                    # all_possible_rews.append(h_reward_hypothesis[possible_action] + self.ind_rew[robot_action])
                    human_only_rew_for_possible_action = 0
                    for r_action in self.possible_actions:
                        if r_action is None:
                            continue
                        if current_state[r_action] > 0:
                            human_only_rew_for_possible_action += (h_reward_hypothesis[possible_action] + self.ind_rew[r_action])
