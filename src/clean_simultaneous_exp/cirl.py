...
class Simultaneous_Cleanup():
    ...
    def step(self, joint_action):
        ...
        if robot_action == human_action and robot_action is not None:
            ...
            if robot_action_weight == 1:
                team_rew, robot_rew, human_rew, robot_action_successful, human_action_successful = self.resolve_heavy_pickup(robot_action)

            # single pick up object
            else:
                team_rew, robot_rew, human_rew, robot_action_successful, human_action_successful = self.resolve_two_agents_same_item(robot_action, human_action)

        else:
            team_rew, robot_rew, human_rew, robot_action_successful, human_action_successful = self.resolve_two_agents_diff_item(robot_action, human_action)
    ...
    def rollout_full_game_joint_optimal(self):
        ...
        # for i in range(10):
            ...
            print(
                f"a:{action}, team = {team_rew}, robot={robot_rew}, human = {human_rew}, total = {team_rew + robot_rew + human_rew}")
    ...
    def rollout_multiround_game_two_agents_old(self, num_rounds):
        total_reward = 0
        human_only_reward = 0
        robot_only_reward = 0
        ...
        if max_prob_idx == self.robot.true_human_rew_idx or second_prob_idx == self.robot.true_human_rew_idx or abs(max_prob - self.robot.beliefs[self.robot.true_human_rew_idx]['prob'] )<= 0.2:
            max_prob_is_correct = True
        return total_reward, human_only_reward, robot_only_reward, multiround_belief_history, reward_for_all_rounds, max_prob_is_correct
    ...
    def rollout_multiround_game_two_agents(self, num_rounds, plot=False):
        ...
        max_prob_is_correct = False
        if max_prob_idx == self.robot.true_human_rew_idx:
            max_prob_is_correct = True

        max_prob_is_close = False
        if max_prob_idx == self.robot.true_human_rew_idx or second_prob_idx == self.robot.true_human_rew_idx or abs(
                max_prob - self.robot.beliefs[self.robot.true_human_rew_idx]['prob']) <= 0.2:
            max_prob_is_close = True
        return total_reward, human_only_reward, robot_only_reward, multiround_belief_history, reward_for_all_rounds, max_prob_is_correct, max_prob_is_close
...
def run_k_rounds(exp_num, task_reward, h_alpha=0.0):
    ...
    robot_rew = {(BLUE, 0): 0,
                 ...
                 (BLUE, 1): 0,
                 (RED, 1): 0}
    human_rew = {(BLUE, 0): np.random.randint(3, 10),
                 (RED, 0): np.random.randint(3, 10),
                 (BLUE, 1): np.random.randint(3, 10),
                 (RED, 1): np.random.randint(3, 10)}
    # human_rew = {(BLUE, 0): np.random.randint(5, 10),
    #              (RED, 0): np.random.randint(-10,-5),
    #              (BLUE, 1): np.random.randint(5, 10),
    #              (RED, 1): np.random.randint(5, 10)}
    ...
    robot = Robot(team_rew, robot_rew, human_rew, starting_objects, robot_knows_human_rew=True, permutes=permutes, vi_type='cvi', is_collaborative_human=False)
    ...
    robot = Robot(team_rew, robot_rew, human_rew, starting_objects, robot_knows_human_rew=False, permutes=permutes, vi_type='cvi', is_collaborative_human=False)
    human = Greedy_Human(human_rew, robot_rew, starting_objects, h_alpha)
    robot.setup_value_iteration()
    env = Simultaneous_Cleanup(robot, human, starting_objects)
    cvi_rew, cvi_human_rew, cvi_robot_rew, multiround_belief_history, reward_for_all_rounds, max_prob_is_correct, max_prob_is_close = env.rollout_multiround_game_two_agents(
        num_rounds=6, plot=False)
    print("CVI final_team_rew = ", cvi_rew)

    robot = Robot(team_rew, robot_rew, human_rew, starting_objects, robot_knows_human_rew=True, permutes=permutes, vi_type='stdvi', is_collaborative_human=False)
    human = Greedy_Human(human_rew, robot_rew, starting_objects, h_alpha)
    robot.setup_value_iteration()
    env = Simultaneous_Cleanup(robot, human, starting_objects)
    ...
    for j in range(len(reward_for_all_rounds)):
        # if optimal_rew < 0:
        #     shifted_reward = reward_for_all_rounds[j] + abs(optimal_rew)
        # elif optimal_rew == 0:
        #     shifted_reward = reward_for_all_rounds[j] + 1
        #     optimal_rew = 1
        # else:
        shifted_reward = reward_for_all_rounds[j]
        percent_opt_each_round.append(max(0,shifted_reward) / optimal_rew)
...
def run_experiment():
    ...
    with Pool(processes=num_exps) as pool:
        ...
        for result in k_round_results:
            ...
            cvi_percent_of_opt_robot, stdvi_percent_of_opt_robot, altruism_case, percent_opt_each_round, max_prob_is_correct, max_prob_is_close = result
    ...
    plt.legend()
    plt.savefig(f"images/cirl_{num_exps}_cdf.png")
    plt.show()
    plt.close()
    ...
    # show the plot
    fig.tight_layout()
    plt.savefig(f"images/cirl_{num_exps}_violin.png")
    plt.show()
    plt.close()
    ...
    fig.tight_layout()
    plt.savefig(f"images/cirl_{num_exps}_bar.png")
    plt.show()
    plt.close()
    ...
    plt.title('Interactive IRL', fontsize=16)
    plt.plot(X, Y, 'k-')
    ...
    plt.legend()
    plt.savefig(f"images/cirl_{num_exps}_by_round_Std.png")
    plt.show()
...
def run_experiment_without_multiprocess():
    ...
    for exp_num in range(num_exps):
        ...
        cvi_percent_of_opt_team, stdvi_percent_of_opt_team, cvi_percent_of_opt_human, stdvi_percent_of_opt_human, \
        cvi_percent_of_opt_robot, stdvi_percent_of_opt_robot, altruism_case, percent_opt_each_round, max_prob_is_correct, max_prob_is_close = exp_num_results
