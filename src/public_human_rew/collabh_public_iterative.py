...
def run_exp_config(start_state, all_colors_list, task_reward, human_rew, h_rho, robot_rew, r_rho, vi_type):
    robot_agent = Robot_Model(robot_rew, all_colors_list, task_reward, [1], vi_type, true_human_reward=human_rew)
...
def run_k_rounds(exp, task_reward, r_rho, h_rho_of_interest):
    ...
    for h_rho in [h_rho_of_interest]:
        ...
        cvi_rew, cvi_human_rew, cvi_robot_rew = run_exp_config(start_state, all_colors_list, task_reward, human_rew,
                                                               h_rho,
                                                               robot_rew, r_rho, 'cvi')
        stdvi_rew, stdvi_human_rew, stdvi_robot_rew = run_exp_config(start_state, all_colors_list, task_reward,
                                                                     human_rew, h_rho,
...
def non_multiprocessed():
    ...
    robotrew_means = [np.round(np.mean(cvi_percents[h_rho_of_interest]), 2), np.round(np.mean(stdvi_percents[h_rho_of_interest]), 2)]
    robotrew_stds = [np.round(np.std(cvi_percents[h_rho_of_interest]), 2), np.round(np.std(stdvi_percents[h_rho_of_interest]), 2)]

    humanrew_means = [np.round(np.mean(cvi_humanrew_percents[h_rho_of_interest]), 2), np.round(np.mean(stdvi_humanrew_percents[h_rho_of_interest]), 2)]
    humanrew_stds = [np.round(np.std(cvi_humanrew_percents[h_rho_of_interest]), 2), np.round(np.std(stdvi_humanrew_percents[h_rho_of_interest]), 2)]
    ...
    fig, ax = plt.subplots(figsize=(5, 5))
    rects1 = ax.bar(ind, robotrew_means, width, yerr=robotrew_stds,
    ...
    fig.tight_layout()
    plt.savefig("collab_public_iterative_100.png")
    plt.show()