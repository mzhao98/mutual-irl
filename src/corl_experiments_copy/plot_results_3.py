...
def plot_cirl_w_hard_rc_3(human_types):
    ...
    for human_type in human_types:
        ...
        for random_human in [False]:
            ...
            for robot_type in robot_types:
                ...
                # experiment_number = f'domain1_5r_3objs5_{robot_type}_{human_type}_human'
                # global_seed = 0
                # if human_type in ['boltz_b1', 'boltz_b1_pmf', 'boltz_binf_pmf'] and robot_type in ['robot_1_birl_bsp_ig', 'robot_2_birl_bsp']:
                global_seed = 1
                if human_type == 'boltz_prag_h_b1_actual_hv_1' and robot_type == 'robot_10_maxent_maxplan':
                    global_seed = 0
                    experiment_number = f'domain1_5r_3objs5_{robot_type}_{human_type}_human'
                else:
                    experiment_number = f'domain1_5r_3objs5_rep2_{robot_type}_{human_type}_human'
                # if 'prag' in human_type:
                ...
                data = get_data(foldername, global_seed, experiment_number, task_type, exploration_type,
                                replan_type, random_human, num_exps)
                aggregate_data_over_rounds = {round_no: [] for round_no in range(-1, n_rounds)}
                seed_to_percent_optimal_rew = {}
                for exp_num in range(num_exps):
                    exp_results = data[exp_num]['results']
                    exp_config = data[exp_num]['config']
                    ...
                    # final_reward_per_round = []
                    seed_to_percent_optimal_rew[seed] = []
                    for round_no in range(-1, n_rounds):
                        final_reward = game_results[round_no]['final_reward']
                        aggregate_data_over_rounds[round_no].append(max(0,final_reward / optimal_rew))
                        seed_to_percent_optimal_rew[seed].append(max(0,final_reward / optimal_rew))
                ...
                mean_aggregate_data_over_rounds = {round_no: np.mean(aggregate_data_over_rounds[round_no]) for
                                                   round_no in range(-1, n_rounds)}
                std_aggregate_data_over_rounds = {round_no: np.std(aggregate_data_over_rounds[round_no]) for
                                                  round_no in range(-1, n_rounds)}

                percents_means = np.array(
                    [np.mean(aggregate_data_over_rounds[round_no]) for round_no in range(0, n_rounds)])
                percents_stds = np.array(
                    [sem(aggregate_data_over_rounds[round_no]) for round_no in range(0, n_rounds)])
            ...
            ax.set_xticklabels([str(elem) for elem in range(1, n_rounds+1)], fontsize=12)
    ...
    plt.savefig(f"exp13_test_{human_types}.png")
...
if __name__ == "__main__":
    ...
    # plot_cirl_w_hard_rc_3(['boltz_b1', 'boltz_b1_pmf'])
    # plot_cirl_w_hard_rc_3(['noiseless', 'boltz_binf_pmf'])
    # plot_ablate_beta()
    # plot_ablate_lambda()
